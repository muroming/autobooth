...
class UserController {
    ...
    fun registerUser(@RequestBody input: String) = try {
        ...
        val token = jwtUtil.generateToken(User(createUserDto.email, createUserDto.password, emptyList()))
        ResponseEntity.ok(UserJwtResponse(userId, createUserDto.firstName, createUserDto.secondName, token))
    } catch (e: Exception) {
        ResponseEntity.status(500).body("Internal server error")
    }
}